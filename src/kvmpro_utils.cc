...
const string
Kvmpro::procstat_files(struct procstat *procstat, struct kinfo_proc *kipp) {
...
const string
Kvmpro::protocol_to_string(const int domain, const int type, const int protocol) {
    switch (domain) {
        case AF_INET:
        case AF_INET6:
            switch (protocol) {
...
const string
Kvmpro::addr_to_string(const struct sockaddr_storage *ss) {
    stringstream out;
    char port_str[7];
...
const string
Kvmpro::escape_json(const string& input) {
